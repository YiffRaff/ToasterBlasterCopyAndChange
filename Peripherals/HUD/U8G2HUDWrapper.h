...
class U8G2HUDWrapper : public HeadsUpDisplay {
  ...
  public:
    U8G2HUDWrapper(u8 scl, u8 sda) {
        ...
        Wire.begin(sda, scl); // Ensure ESP32 I2C is initialized before u8g2

        // Auto-scan I2C bus for the OLED address and set it if needed.
        // Many SSD1306 modules use 0x3C, some use 0x3D. u8g2 expects the
        // 7-bit address shifted left by 1 (hence * 2).
        uint8_t foundAddr = 0;
        ...
        if (foundAddr != 0) {
            printf("HUD I2C device found at 0x%02X\n", foundAddr);
            if (foundAddr != 0x3C) {
                u8g2Renderer->setI2CAddress(foundAddr * 2);
                printf("u8g2 I2C address set to 0x%02X (shifted)\n", foundAddr * 2);
            }
        } else {
            printf("No I2C devices found on HUD bus\n");
        }
